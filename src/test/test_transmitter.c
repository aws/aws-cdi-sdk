...
/// Container for any data we need separate for each Tx thread.
typedef struct  {
    CdiSgList sgl;        ///< Used by RightSizeSgl.
    CdiSglEntry entry[5]; ///< Used by RightSizeSgl.
} TestTxThreadLocalData;
...
static int cdi_min(int a, int b) { return a > b ? b : a; }

/**
 * Return a new, static SGL of smaller buffer size but pointing to the same buffer.
 *
 * @param source_sgl_ptr Pointer to the source SGL.
 * @param new_size Size of the new SGL to be constructed.
 * @param tx_data_ptr Pointer to thread-local data.
 *
 * @return Static SGL of specified size.
 */
static CdiSgList* RightSizeSgl(CdiSgList const* source_sgl_ptr, int new_size, TestTxThreadLocalData* tx_data_ptr)
{
    if (new_size > source_sgl_ptr->total_data_size) {
        CDI_LOG_THREAD(kLogError, "Cannot upsize SGL");
        return NULL;
    }

    // Construct a five-entry SGL. (No more than five needed, see InitPoolSgl in run_test_generic.c.)
    memset(tx_data_ptr, 0, sizeof(*tx_data_ptr));
    for (int i = 0; i < 4; i++) {
        tx_data_ptr->entry[i].next_ptr = &tx_data_ptr->entry[i + 1];
    }
    tx_data_ptr->sgl.sgl_head_ptr = &tx_data_ptr->entry[0];

    // Update fields based on source SGL.
    tx_data_ptr->sgl.total_data_size = new_size;
    tx_data_ptr->sgl.internal_data_ptr = source_sgl_ptr->internal_data_ptr;
    CdiSglEntry *src_entry_ptr = source_sgl_ptr->sgl_head_ptr;
    for (CdiSglEntry* entry_ptr = tx_data_ptr->sgl.sgl_head_ptr; 0 != new_size; entry_ptr = entry_ptr->next_ptr) {
        assert(src_entry_ptr);
        entry_ptr->address_ptr = src_entry_ptr->address_ptr;
        entry_ptr->size_in_bytes = cdi_min(new_size, src_entry_ptr->size_in_bytes);
        entry_ptr->internal_data_ptr = src_entry_ptr->internal_data_ptr;
        new_size -= entry_ptr->size_in_bytes;
        src_entry_ptr = src_entry_ptr->next_ptr;
        tx_data_ptr->sgl.sgl_tail_ptr = entry_ptr;
    }
    tx_data_ptr->sgl.sgl_tail_ptr->next_ptr = NULL;

    return &tx_data_ptr->sgl;
}

/**
 * Check all requirements for the payload SGL generated by InitPoolSgl in run_test_generic.c.
 *
 * @param sgl_ptr Pointer to SGL to validate.
 * @param buffer_type The configured buffer type.
 * @param expected_size Expected buffer size.
 *
 * @return True if and only if the SGL is as expected.
 */
static bool ValidateSgl(CdiSgList const* sgl_ptr, CdiBufferType buffer_type, int expected_size)
{
    ...
    for (CdiSglEntry* entry_ptr = sgl_ptr->sgl_head_ptr; NULL != entry_ptr; entry_ptr = entry_ptr->next_ptr) {
        if (0 >= entry_ptr->size_in_bytes) {
            CDI_LOG_THREAD(kLogFatal, "invalid SGL entry (negative size)");
            sgl_ok = false;
        }
        total_size += entry_ptr->size_in_bytes;
        num_entries++;
    }

    if (kCdiLinearBuffer == buffer_type && 1 != num_entries) {
        CDI_LOG_THREAD(kLogFatal, "buffer type is LINEAR but SGL has more than one entry");
        sgl_ok = false;
    }
    if (num_entries > 5) {
        // We rely on this in RightSizeSgl. See InitPoolSgl in run_test_generic.
        CDI_LOG_THREAD(kLogFatal, "More than 5 SGL entries");
        sgl_ok = false;
    }
    if (total_size != sgl_ptr->total_data_size) {
        CDI_LOG_THREAD(kLogFatal, "SGL entries don't add up to total_data_size");
        sgl_ok = false;
    }
    if (expected_size != sgl_ptr->total_data_size) {
        CDI_LOG_THREAD(kLogFatal, "buffer size is [%d], expected [%d]", sgl_ptr->total_data_size, expected_size);
        sgl_ok = false;
    }
    return sgl_ok;
}
                                         ...
                                         int ptp_rate_count, bool resend)
{
    ...
    TestTxThreadLocalData* tx_data_ptr = connection_info_ptr->local_data_ptr;
    ...
    if (!got_error) {
        got_error = !CdiPoolGet(stream_info_ptr->tx_pool_handle, (void**)&sgl_ptr);
        ...
        user_data_ptr->tx_payload_sgl_ptr = sgl_ptr;
    }

    // Right-size the SGL when the payload size changes from one payload to the next (RIFF-sourced payloads).
    if (!got_error) {
        sgl_ptr = RightSizeSgl(sgl_ptr, stream_info_ptr->next_payload_size, tx_data_ptr);
        got_error = NULL == sgl_ptr;
    }

    // Sanity-check the SGL.
    if (!got_error) {
        got_error = !ValidateSgl(sgl_ptr, test_settings_ptr->buffer_type, stream_info_ptr->next_payload_size);
    }

    if (!resend && !got_error) {
        // Either load the next payload from file, or update the first word of the buffer if we are using patterns.
        got_error = !GetNextPayloadDataSgl(connection_info_ptr, stream_settings_ptr, payload_count,
        ...
        if (!got_error) {
            ...
            if (time_to_sleep > rate_period_microseconds) {
                CDI_LOG_THREAD(kLogWarning, "Connection[%s] sleep time of [%"PRIu64"]us is greater than the "
                    ...
                    CdiGetEmptyStringIfNull(connection_info_ptr->test_settings_ptr->connection_name_str),
...
static bool TestTxSendTestData(TestConnectionInfo* connection_info_ptr)
{
    ...
    if (!got_error) {
        // Done signal timeout. Wait a little while for pending payloads to receive ACKs before aborting.
        uint32_t timeout_ms = TX_ALL_DONE_TIMEOUT_MS;
#ifdef DISABLE_RATE_TIMEOUT_FOR_DEBUG
...
CDI_THREAD TestTxCreateThread(void* arg_ptr)
{
    ...
    connection_info_ptr->local_data_ptr = &tx_data;
    ...
    if (!got_error) {
        ...
        if (kProtocolTypeRaw == test_settings_ptr->connection_protocol) {
            ...
            for (int i = 0; i < stream_count && !got_error; i++) {
                ...
                if (kCdiStatusOk != CdiAvmTxStreamEndpointCreate(connection_info_ptr->connection_handle,
    ...
    if (connection_info_ptr->connection_handle) {
        ...
        // It is not strictly necessary to explicitly call CdiAvmStreamEndpointDestroy in this use case, but we want to
        // exercise the API.
        if (test_settings_ptr->multiple_endpoints) {
            int stream_count = test_settings_ptr->number_of_streams;
            for (int i = 0; i < stream_count; i++) {
                CdiReturnStatus status = CdiAvmStreamEndpointDestroy(connection_info_ptr->tx_stream_endpoint_handle_array[i]);
                if (kCdiStatusOk != status) {
                    got_error = true;
                }
